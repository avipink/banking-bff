...
## Role
Frontend aggregation layer (Backend-for-Frontend). Spring Boot REST service (port 8080).
Orchestrates calls to Core services and composes responses for mobile/web clients.

## Ecosystem
- Shared contracts: ../banking-contracts
- Orchestrator/specs: ../banking-platform/specs/
- Upstream: ../accounts-core-svc (port 8081), ../payments-core-svc (port 8082)

## Architecture Rules
- BFF NEVER owns domain data — it only aggregates from Core services
- MUST use contract types from banking-contracts for all inter-service data shapes
- No business logic — orchestration and composition only
- Two patterns coexist in this repo to demonstrate the coupling problem:
  - legacy/: WRONG — duplicated DTOs that mirror contract types (anti-pattern)
  - clean/:  CORRECT — composes contract types directly (reference implementation)

## Package Structure
com.digitalbank.bff/
├── legacy/   — LegacyDashboardController + duplicated DTOs (anti-pattern demo)
└── clean/    — DashboardController + DashboardView composing contract types

## API Surface (port 8080)
- GET  /api/v1/dashboard               → DashboardView (clean pattern)
- GET  /api/v1/dashboard/accounts/{id} → AccountDetailView
- POST /api/v1/dashboard/transfer      → PaymentResponse
- GET  /api/v1/legacy/dashboard        → Legacy response (anti-pattern demo)

## Upstream Config (application.yml)
- accounts-service.base-url: http://localhost:8081
- payments-service.base-url: http://localhost:8082

## Build & Run
- ./gradlew bootRun       (starts on port 8080, requires both core services running)
- ./gradlew test
- Swagger UI: http://localhost:8080/swagger-ui.html
- Depends on banking-contracts via: includeBuild("../banking-contracts")
